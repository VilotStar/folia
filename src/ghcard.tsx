...
const fetchGhRepo = async (fn:string) => (await fetch(`https://api.github.com/repos/${fn}`,{credentials:"omit",mode:"cors"})).json()
...
const GhCard: Component<{fn:string}> = (props) => {
    ...
    return (
        <a class="w-96 block mb-4" href={`https://github.com/${props.fn}`} >
            {repoData.loading ? <div>
                <h1>Loading...</h1>
            </div> : <div class="p-4 rounded-xl bg-pink-400/30 border-pink-500/30 border transition-colors hover:bg-pink-400/70" >
				<div class="flex">
					{/* top */}
					<div class="flex-1">
						{/* left // strech */}
					...
					<div class="flex flex-col items-end" >
						{/* right */}
						<p class="dark:text-neutral-300 text-neutral-900 font-semibold" >⭐{repoData().stargazers_count}</p>
				...
				<div class="flex flex-wrap space-x-3 mt-2">
					...
					<For each={repoData().topics} fallback={<></>}>
						{(topic) => <div class="bg-fuchsia-300/90 w-fit px-2 rounded-full text-black text-center border border-fuchsia-500/60" >{topic}</div>}
					</For>
				</div>
            </div>}
        </a>
